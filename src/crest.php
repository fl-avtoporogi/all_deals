 ...
 /*   public static function call($method, $params = [])
    {
        ...
        $attempt = 0;
        $logPath = __DIR__ . '/crest_call_error.log'; // Путь к файлу журнала ошибок
        ...
        while ($attempt < $maxAttempts) {
            try {
                ...
                if (isset($result['result'])) {
                    ...
                    throw new Exception("Ошибка API: " . $result['error_description']);
                }
            } catch (Exception $e) {
                $errorMsg = $e->getMessage();

                // Проверяем, содержит ли сообщение об ошибке "Too many requests"
                if (stripos($errorMsg, 'Too many requests') !== false) {
                    // Логируем ошибку
                    $errorMessage = date('Y-m-d H:i:s') . ' - Ошибка API: ' . $errorMsg . "\n";
                    $errorMessage .= "Method: " . $method . "\n";
                    $errorMessage .= "Params: " . json_encode($params) . "\n";
                    file_put_contents($logPath, $errorMessage, FILE_APPEND);

                    // Задержка перед следующей попыткой (можно увеличить экспоненциальную задержку при необходимости)
                    usleep(150000); // 0.15 секунды

                    $attempt++;
                    if ($attempt >= $maxAttempts) {
                        throw new Exception("Превышено максимальное количество попыток для метода $method.");
                    }
                    // Продолжаем цикл для повторной попытки
                } else {
                    // Для других ошибок не выполняем повторные попытки, сразу выбрасываем исключение
                    $errorMessage = date('Y-m-d H:i:s') . ' - Необрабатываемая ошибка API: ' . $errorMsg . "\n";
                    $errorMessage .= "Method: " . $method . "\n";
                    $errorMessage .= "Params: " . json_encode($params) . "\n";
                    file_put_contents($logPath, $errorMessage, FILE_APPEND);
        ...
        return null; // Возвращается null в случае неудачи после всех попыток
    }*/
